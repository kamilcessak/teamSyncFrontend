...
export function DashboardPage() {
  const { data: stats, isLoading: statsLoading, isError: isStatsError, refetch: refetchStats } = useDashboardStats();
  const { data: recentTasks, isLoading: tasksLoading, isError: isTasksError, refetch: refetchTasks  } = useRecentTasks();
  const { data: upcomingEvents, isLoading: eventsLoading, isError: isEventsError, refetch: refetchEvents } = useUpcomingEvents();

  const hasAnyError = isStatsError || isTasksError || isEventsError;
  ...
  return (
    ...
    <div className="space-y-8">
      ...
      {hasAnyError ? (
        <ErrorState retryAction={handleRetryAll} />
      ) : (
        <>
